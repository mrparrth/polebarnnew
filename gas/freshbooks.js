function onOpen() {
  SpreadsheetApp.getUi().createMenu('⚙️Custom Menu')
...
function getAccountId() {
  ...
  console.log("Your business id is " + response["response"]["business_memberships"][1]["business"]["id"]);
}
...
function createInvoiceJson(clientData) {
  ...
  individualItems["description"] = `Project ${clientData['project-number']} - ${clientData['project']}${clientData['upwork-job'] ? " - Upwork Job" : ""}`
  ...
  scopes.forEach((scope) => {
    ...
    individualItems["qty"] = 1;
    individualItems["unit_cost"] = { amount: parseFloat(scope.rate.replace(/[$,]/g, '')) || 0, code: 'USD' }
...
function getAllServices() {
  const response = fbGetRequest_(`https://api.freshbooks.com/comments/business/${BUSINESS_ID}/services`)
...
function createFreshbooksInvoice(data) {
  ...
  individualItems["type"] = 0;
  individualItems["name"] = "Info";
  individualItems["description"] = `Project ${data['projectid']} - Backwoods Buildings & Truss LLC - Open Contract`
  individualItems["qty"] = 1;
  individualItems["unit_cost"] = { amount: "0", code: 'USD' };
  allItems.push(individualItems);
  ...
  individualItems["qty"] = 1;
  individualItems["unit_cost"] = { amount: `${parseFloat(data['projectpricing'].replace(/[$,]/g, '') || 0)}`, code: 'USD' }
  ...
  let response = fbPostRequest_(`https://api.freshbooks.com/accounting/account/${ACCOUNT_ID}/invoices/invoices`, finalDict);
